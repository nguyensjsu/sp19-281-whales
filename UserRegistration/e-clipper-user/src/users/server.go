...
var mongodb_server = `mongodb://34.213.86.58:27017,50.112.86.208:27017,52.33.110.43:27017`
...
func initRoutes(router *mux.Router, formatter *render.Render) {
	...
	router.HandleFunc("/user/{clipperid}", GetUserByClipperID).Methods("GET")
...
func checkPing(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		message := "E-Clipper Users API Server is Working on machine: " + getSystemIp()
...
func GetUserByClipperID(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(req)
	var m User
    _ = json.NewDecoder(req.Body).Decode(&m)		
    fmt.Println("Get data of user: ", params["clipperid"])
	session, err := mgo.Dial(mongodb_server)
    if err != nil {
		message := struct {Message string}{"Some error occured while connecting to database!!"}
		w.WriteHeader(http.StatusInternalServerError)
    ...
    session.SetMode(mgo.Monotonic, true)
    c := session.DB(mongodb_database).C(mongodb_collection)
    query := bson.M{"clipperid" : params["clipperid"]}
    var result bson.M
    err = c.Find(query).One(&result)
    if err != nil && err != mgo.ErrNotFound{
		message := struct {Message string}{"Some error occured while querying to database!!"}
...
func CreateUser(w http.ResponseWriter, req *http.Request) {
    ...
    if person.Clipperflag  == "Y"{
	   query := bson.M{"clipperid" : person.Clipperid}
	    var result bson.M
	   err = c.Find(query).One(&result)
	  if err != nil && err != mgo.ErrNotFound{
		message := struct {Message string}{"Some error occured while querying to database!!"}
		...
		message := struct {Message string}{"The clipperid is already associated with an account"}
		w.WriteHeader(http.StatusConflict)
		json.NewEncoder(w).Encode(message)
		return
	}
    }else if person.Clipperflag  == "N"{

    	   rand.Seed(time.Now().UnixNano())
           var max int64 = 9999999999
           var min int64 = 1000000000
           uniquID := (rand.Int63n(max-min) + min)
           person.Clipperid = strconv.FormatInt(uniquID, 10)    
